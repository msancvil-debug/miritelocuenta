...
# Credenciales de Email (Respaldo)
...
# Colección garantizada de fotografías HD de plató, televisión y espectáculos (Cero fondos grises)
...
def descargar_foto_fondo():
    """Descarga una fotografía de estudio o plató garantizada, sin riesgo de fondos grises."""
    print("🖼️ Seleccionando fotografía de fondo en alta calidad...")
    url_foto = random.choice(FOTOS_ESTUDIO_GARANTIZADAS)
    try:
        r = requests.get(url_foto, headers=HEADERS_BROWSER, timeout=12)
        if r.status_code == 200 and len(r.content) > 10000:
            img = Image.open(BytesIO(r.content)).convert("RGBA")
            print("✅ Fotografía cargada con éxito.")
            return img
    except Exception as e:
        print(f"⚠️ Error descargando foto: {e}")
    
    # Respaldo seguro si falla la red externa
    return Image.new("RGBA", (1200, 630), (50, 40, 60, 255))

def crear_imagen_destacada(titulo):
    width, height = 1200, 630
    bg_img = descargar_foto_fondo()

    # Redimensionar y recortar para ajustar exactamente a 1200x630
    ...
    # 2. FALDÓN AMARILLO INFERIOR (Dejando el 65% superior libre para ver la foto)
...
def publicar_en_wordpress(titulo, contenido_html, ruta_imagen):
    if WP_URL and WP_USER and WP_APP_PASS:
        try:
            ...
            # Insertar la imagen en la primera línea para que Metricool la detecte por RSS
            html_final = f"""
